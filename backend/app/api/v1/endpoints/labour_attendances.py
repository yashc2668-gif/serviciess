...
router = APIRouter(prefix="/labour-attendances", tags=["Labour Attendance"])


@router.get("", response_model=PaginatedResponse[LabourAttendanceOut])
@router.get("/", response_model=PaginatedResponse[LabourAttendanceOut], include_in_schema=False)
def list_all_labour_attendances(
...
@router.post("", response_model=LabourAttendanceOut, status_code=201)
@router.post(
    "/",
    response_model=LabourAttendanceOut,
    status_code=201,
    include_in_schema=False,
)
def create_new_labour_attendance(
...
@router.get("/{attendance_id}", response_model=LabourAttendanceOut)
def get_single_labour_attendance(
    attendance_id: int,
    db: Session = Depends(get_db),
...
@router.put("/{attendance_id}", response_model=LabourAttendanceOut)
def update_existing_labour_attendance(
    attendance_id: int,
...
@router.post("/{attendance_id}/submit", response_model=LabourAttendanceOut)
def submit_existing_labour_attendance(
    attendance_id: int,
...
@router.post("/{attendance_id}/approve", response_model=LabourAttendanceOut)
def approve_existing_labour_attendance(
    attendance_id: int,
    payload: LabourAttendanceTransitionRequest | None = None,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_permissions("attendance:approve")),
